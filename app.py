...
<style>
  ...
  *{box-sizing:border-box;} html,body{margin:0;padding:0;}
  ...
  .board-scroll{ overflow-x:auto; overflow-y:hidden; height:640px;
    background-image:radial-gradient(#DFD6BE 1px, transparent 1px); background-size:14px 14px; }
  .board-canvas{ position:relative; height:100%; min-width:2620px; }
...
with st.sidebar:
    ...
    if is_editor:
        ...
        labels = [f'{data["etapas"][eid]["nome"]}' for eid in etapa_ids]
        idx = st.selectbox("Etapa", range(len(etapa_ids)), format_func=lambda i: labels[i])
        eid = etapa_ids[idx]
        ...
        if submitted:
            ...
            if ok:
                st.success("Salvo! O painel público atualiza em cerca de 1 minuto (o Streamlit Cloud redetecta o commit).")
                st.rerun()
            else:
